...
Feature: MySQL Source - Design time scenarios (macro)
  ...
  Scenario: Verify user should be able to validate plugin with macros for Basic section
    ...
    And Click on the Macro button of Property: "referenceName" and set the value to: "DriverName"
    And Click on the Macro button of Property: "database" and set the value to: "DriverName"
    Then Enter textarea plugin property: "importQuery" with value: "selectQuery"
    Then Validate "MySQL" plugin properties
    Then Enter input plugin property: "referenceName" with value: "sourceRef"
    Then Replace input plugin property: "database" with value: "databaseName"
